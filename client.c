...
void *receive_handler(void *arg) {
    ...
    while (keep_running) {
        ...
        if (receive > 0) {
            ...
            if (keep_running) { // Avoid message if we initiated the exit
                printf("\n%s%sSYSTEM: Server disconnected.\n", ANSI_CURSOR_START, ANSI_CLEAR_LINE);
            }
            keep_running = 0; // Signal main thread to exit
            break;
        } else { // receive < 0
             if (errno == EINTR && keep_running) { // Interrupted by signal (like Ctrl+C handled by us)
                continue;
             } else if (keep_running) { // Only report error if we weren't expecting to stop
...
int main(int argc, char *argv[]) {
        ...
        if (fgets(message, BUFFER_SIZE, stdin) != NULL) {
            ...
            if (strlen(message) > 0) {
                ...
                if (write(sockfd, message, strlen(message)) < 0) {
                    ...
                    if (errno == EPIPE) { // Server likely closed connection
                         if (keep_running) { // Avoid message if we initiated the exit
    ...
    // Sprzątanie
    if (sockfd > 0) { // Check if socket wasn't already closed by handler
        printf("\nDisconnecting...\n");
