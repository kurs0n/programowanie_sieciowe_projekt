...
void send_to_client(int sockfd, const char *message) {
    if (write(sockfd, message, strlen(message)) < 0) {} // Ignore write errors silently for now
}
        ...
        if (receive > 0) {
            ...
            if (buffer[0] == '/') {
                ...
                // Extract command part
                 ...
                 if (cmd_len >= BUFFER_SIZE) cmd_len = BUFFER_SIZE - 1;
                ...
                // Extract argument part if exists
                 if (has_argument) {
                     ...
                     size_t arg_len = strlen(arg_start);
                      if (arg_len >= BUFFER_SIZE) arg_len = BUFFER_SIZE - 1;
                ...
                if (strcmp(command, "/users") == 0 || strcmp(command, "/who") == 0) {
                    ...
                    for (int i = 0; i < MAX_CLIENTS; ++i) {
                        if (clients[i] && strcmp(clients[i]->room, cli->room) == 0) {
                            if (!first_user) current_len += snprintf(user_list_buffer + current_len, BUFFER_SIZE - current_len, ", ");
                            current_len += snprintf(user_list_buffer + current_len, BUFFER_SIZE - current_len, "%s", clients[i]->nickname);
                            if (current_len >= BUFFER_SIZE - (NICK_LEN + 5)) { snprintf(user_list_buffer + current_len, BUFFER_SIZE - current_len, "..."); break; }
                            first_user = false;
                        }
                    }
                     if (current_len < BUFFER_SIZE - 3) strcat(user_list_buffer, "]\n");
                     else strcpy(user_list_buffer + BUFFER_SIZE - 4, "]\n");
                    pthread_mutex_unlock(&clients_mutex);
                    ...
                    for (int i = 0; i < MAX_CLIENTS; ++i) { 
                         if (clients[i]) {
                            bool found = false;
                            for (int j = 0; j < unique_room_count; ++j) if (strcmp(clients[i]->room, unique_rooms[j]) == 0) { found = true; break; }
                            if (!found && unique_room_count < MAX_CLIENTS) { strncpy(unique_rooms[unique_room_count], clients[i]->room, ROOM_LEN -1); unique_rooms[unique_room_count][ROOM_LEN -1] = '\0'; unique_room_count++; }
                        }
                    }
                    current_len += snprintf(room_list_buffer + current_len, BUFFER_SIZE - current_len, "SYSTEM: Available rooms: ");
                    for (int j = 0; j < unique_room_count; ++j) { /* ... count users and build string ... */
                        int users_in_room = 0;
                        for (int i = 0; i < MAX_CLIENTS; ++i) if (clients[i] && strcmp(clients[i]->room, unique_rooms[j]) == 0) users_in_room++;
                        if (!first_room) current_len += snprintf(room_list_buffer + current_len, BUFFER_SIZE - current_len, ", ");
                        current_len += snprintf(room_list_buffer + current_len, BUFFER_SIZE - current_len, "%s (%d)", unique_rooms[j], users_in_room);
                        if (current_len >= BUFFER_SIZE - (ROOM_LEN + 10)) { snprintf(room_list_buffer + current_len, BUFFER_SIZE - current_len, "..."); break; }
                        first_room = false;
                    }
                     pthread_mutex_unlock(&clients_mutex);
                     if (unique_room_count == 0) strcat(room_list_buffer, "None");
                     if (current_len < BUFFER_SIZE - 2) strcat(room_list_buffer, "\n"); else strcpy(room_list_buffer + BUFFER_SIZE - 3, "\n");
                ...
                // *** NEW: /topic command ***
                    ...
                    if (strlen(argument) == 0) { // No argument: Show topic
                        if (topic_idx != -1) {
                           sprintf(message_buffer, "SYSTEM: Topic for '%s' is: %s\n", cli->room, room_topics[topic_idx].topic);
                        } else {
                        ...
                        pthread_mutex_unlock(&room_topics_mutex);
                        send_to_client(cli->sockfd, message_buffer);
                    } else { // Argument provided: Set topic
                         if (topic_idx != -1) {
                ...
                // *** END NEW: /topic command ***
                    ...
                    if (strlen(argument) > 0 && strlen(argument) < NICK_LEN) {
                        char old_nick[NICK_LEN]; strncpy(old_nick, cli->nickname, NICK_LEN); old_nick[NICK_LEN - 1] = '\0';
                        // TODO: Add nick uniqueness check
                        pthread_mutex_lock(&clients_mutex); strncpy(cli->nickname, argument, NICK_LEN - 1); cli->nickname[NICK_LEN - 1] = '\0'; pthread_mutex_unlock(&clients_mutex);
                        sprintf(message_buffer, "SYSTEM: Your nick changed to '%s'.\n", cli->nickname); send_to_client(cli->sockfd, message_buffer);
                        sprintf(message_buffer, "SYSTEM: User '%s' is now known as '%s' in room '%s'.\n", old_nick, cli->nickname, cli->room); broadcast_system_room(message_buffer, cli->room);
                    ...
                    if (strlen(argument) > 0 && strlen(argument) < ROOM_LEN) {
                        if(strcmp(cli->room, argument) == 0) {
                             sprintf(message_buffer, "SYSTEM: You are already in room '%s'.\n", cli->room); send_to_client(cli->sockfd, message_buffer);
                        } else {
                            char old_room[ROOM_LEN]; strncpy(old_room, cli->room, ROOM_LEN); old_room[ROOM_LEN - 1] = '\0';
                            sprintf(message_buffer, "SYSTEM: User '%s' has left room '%s'.\n", cli->nickname, old_room); broadcast_system_room(message_buffer, old_room);
                            pthread_mutex_lock(&clients_mutex); strncpy(cli->room, argument, ROOM_LEN - 1); cli->room[ROOM_LEN - 1] = '\0'; pthread_mutex_unlock(&clients_mutex);
                            ...
                            sprintf(message_buffer, "SYSTEM: User '%s' has joined room '%s'.\n", cli->nickname, cli->room); broadcast_system_room(message_buffer, cli->room);
            ...
            leave_flag = 1;
        } else { // Błąd niekrytyczny
             if (errno != EAGAIN && errno != EWOULDBLOCK) { perror("ERROR: recv failed"); leave_flag = 1; }
        }
    }
...
int main(int argc, char *argv[]) {
    if (argc != 2) { /* ... usage message ... */ fprintf(stderr, "Usage: %s <port>\n", argv[0]); exit(EXIT_FAILURE); }
    ...
    if(lobby_idx != -1) { // Should succeed if MAX_ROOMS > 0
    ...
    listen_sockfd = socket(AF_INET, SOCK_STREAM, 0); if (listen_sockfd < 0) { perror("ERROR opening socket"); exit(EXIT_FAILURE); }
    int opt = 1; if (setsockopt(listen_sockfd, SOL_SOCKET, SO_REUSEADDR, (char *)&opt, sizeof(opt)) < 0) { perror("ERROR on setsockopt"); }
    memset((char *)&serv_addr, 0, sizeof(serv_addr)); serv_addr.sin_family = AF_INET; serv_addr.sin_addr.s_addr = INADDR_ANY; serv_addr.sin_port = htons(port);
    if (bind(listen_sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) { perror("ERROR on binding"); close(listen_sockfd); exit(EXIT_FAILURE); }
    if (listen(listen_sockfd, 5) < 0) { perror("ERROR on listen"); close(listen_sockfd); exit(EXIT_FAILURE); }
    ...
    while (1) {
        ...
        if (conn_sockfd < 0) { perror("ERROR on accept"); continue; }
        ...
        if (client_count >= MAX_CLIENTS) {
            ...
            send_to_client(conn_sockfd, "SYSTEM: Server is full.\n"); close(conn_sockfd); continue;
        }
        pthread_mutex_unlock(&clients_mutex); // Unlock before malloc

        client_t *cli = (client_t *)malloc(sizeof(client_t));
        if (!cli) { perror("ERROR: malloc failed"); close(conn_sockfd); continue; }
        cli->address = cli_addr; cli->sockfd = conn_sockfd;
        sprintf(cli->nickname, "User%d", conn_sockfd); strcpy(cli->room, DEFAULT_ROOM);

        add_client(cli); // Handles its own mutex
        if (pthread_create(&cli->tid, NULL, &handle_client, (void *)cli) != 0) {
             perror("ERROR creating thread"); remove_client(cli->sockfd); free(cli); close(conn_sockfd);
        }
    }
